...
export const assignmentService = {
  // Assignments
  getAssignments: async (params?: { course_id?: string; page?: number; size?: number }): Promise<{
    ...
    const { course_id, page = 1, size = 10 } = params || {};
    const queryParams = new URLSearchParams({
      page: page.toString(),
      size: size.toString(),
      ...(course_id && { course_id })
    });
    const response = await apiClient.get(`/api/assignments/?${queryParams}`);
